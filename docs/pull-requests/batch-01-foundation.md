...
- [x] `ruff`, `mypy`, `eslint`, `prettier` run from the repo root. _(configs in place + `make`
      targets; run after `make setup` installs deps)_
...
- [x] PRs trigger lint + type-check + tests for the changed app only.
- [x] Backend tests run against a real Postgres+pgvector service container.
- [x] CI fails on lint/type/test errors.
- [x] Status badge in README. _(links to the CI workflow on `SagiDahari/job-hunter`)_

**Review checklist.**

- [x] No secrets in workflow; uses GitHub OIDC/secrets where needed.
- [x] Caching configured for deps (pip, npm). _(uv cache + npm cache via committed lockfiles)_
